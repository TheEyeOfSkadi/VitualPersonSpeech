...
namespace VitualPersonSpeech.Utils.KnowledgeBaseUtils
{
    public class ParthenonKnowledgeBase
    {
        ...
        public void DownloadAllKnowledgeBaseFile()
        {
                        ...
                        foreach (IndustryQAChatReply industryQAChatReply in industryQAChatReplyList)
                        {
                            if (!string.IsNullOrEmpty(industryQAChatReply.audioFileSource))
                            {
                                ...
                                if (!File.Exists(localAudioFileFullName))
                                {
                                    ...
                                    HttpUtils.DownloadFile(string.Format(_downloadFileAPIUrl, industryQAChatReply.audioFileSource), localAudioFileFullName);
                                }
                                else
                                {
                            ...
                            foreach (IndustryQAChatReplyScene industryQAChatReplyScene in industryQAChatReply.industryQAChatReplyScenes)
                            {
                                if (!string.IsNullOrEmpty(industryQAChatReplyScene.fileSourceUrl))
                                {
                                    ...
                                    if (!File.Exists(localReplySceneFileName))
                                    {
                                        ...
                                        HttpUtils.DownloadFile(string.Format(_downloadFileAPIUrl, industryQAChatReplyScene.fileSourceUrl), localReplySceneFileName);
                                    }
                                    else
                                    {
        ...
        public ResultMsg AnswerQuest(string questionStr)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    OnSendMsgEvent(ResultMsg.Info($"Parthenon KnowledgeBase 接口回答问题请求,结果{response.Content}"));
                    JObject resultJObject = JObject.Parse(response.Content);
        ...
        public IndustryQAChatReply GetIndustryQAChatReplyById(int id)
        {
            ...
            {
                OnSendMsgEvent(ResultMsg.Info($"当前Parthenon KnowledgeBase 含有此回答Id，Id：{id}，Content:{industryQAChatReplyDic[id].replyContent}"));
                return industryQAChatReplyDic[id];
            }
            else
            {
                OnSendMsgEvent(ResultMsg.Info($"当前Parthenon KnowledgeBase 不含有此回答Id，Id：{id}，更新所有回答信息"));
                DownloadAllKnowledgeBaseFile();

                if (industryQAChatReplyDic.ContainsKey(id))
                {
                    OnSendMsgEvent(ResultMsg.Info($"更新后Parthenon KnowledgeBase 含有此回答Id，Id：{id}，Content：{industryQAChatReplyDic[id].replyContent}"));
                    return industryQAChatReplyDic[id];
                }
                else
                {
                    OnSendMsgEvent(ResultMsg.Info($"更新后Parthenon KnowledgeBase 不含有此回答Id，Id：{id}，返回空数据"));
                    return null;
                }
            }
        }
